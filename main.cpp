...
int main()
{
  string ships, firstShipname, lastShipname;
  cout << "Enter name of ship: ";
  cin >> ships;

  ifstream infile(ships.c_str());

  if(infile.is_open())
  {
    string currentCruise;

    getline(infile, currentCruise);
    firstShipname = currentCruise;
    ...
    {
      getline(infile, currentCruise);
      if(currentCruise.size() != 0)
      {
        countOfFile++;
        lastShipname = currentCruise;
      }
    }
    cout << countOfFile << endl;
    cout << firstShipname << endl;
    cout << lastShipname << endl;
  }

  else
  {
    cout << "The file " << ships << " is not found!" << endl;
  }
